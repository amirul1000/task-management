   ...
   if(isset($_POST['submit']))
  	{
		...
		if($statementMatch->rowCount() > 0) {
          ...
          {
                $query  = "INSERT INTO `users` SET name = ?,username=?,password=?,email = ?,address=?,latitude=?,longitude=?,status=?";
                $parameters = array($_POST['name'],$_POST['username'],$_POST['password'],$_POST['email'],$_POST['address'],$_POST['latitude'],$_POST['longitude'],$_POST['status']);
                  ...
                  if(!in_array($ext,$allowed_filetypes))
                    ...
                    }else
                    {
                        ...
                        $query  = "INSERT INTO `users` SET name = ?,username=?,password=?,email = ?,address=?,avatar=?,latitude=?,longitude=?,status=?";
                        $parameters = array($_POST['name'],$_POST['username'],$_POST['password'],$_POST['email'],$_POST['address'],$imageName,$_POST['latitude'],$_POST['longitude'],$_POST['status']);
