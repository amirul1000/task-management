   ...
   if(isset($_POST['submit']))
  	{
		...
		if($statementMatch->rowCount() > 0) {
          ...
          {
                $query  = "UPDATE  `users` SET name = ?,username=?,password=?,email = ?,address=?,latitude=?,longitude=?,status=? where id=?";
                $parameters = array($_POST['name'],$_POST['username'],$_POST['password'],$_POST['email'],$_POST['address'],$_POST['latitude'],$_POST['longitude'],$_POST['status'],$user_id);
                  ...
                  if(!in_array($ext,$allowed_filetypes))
                    ...
                    }else
                    {
                       ...
                       $query  = "UPDATE `users` SET name = ?,username=?,password=?,email = ?,address=?,avatar=?,latitude=?,longitude=?,status=? where id=?";
                       $parameters = array($_POST['name'],$_POST['username'],$_POST['password'],$_POST['email'],$_POST['address'],$imageName,$_POST['latitude'],$_POST['longitude'],$_POST['status'],$user_id);
						...
						<div class="portlet-content">
							...
							<form id="validate-basic" action="" data-validate="parsley" method="post" class="form parsley-form ajax_form" enctype="multipart/form-data">
								...
								<div class="form-group">
									...
									<input type="text" id="password" name="password" class="form-control" data-required="true" value="<?php echo $row_user['password'];?>">
								</div>
