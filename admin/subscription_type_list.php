...
<div class="portlet box green">
			  ...
			  <form name="search_frm" id="search_frm" method="post">
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					<TD  nowrap="nowrap">
					  <?php
						  $hash    =  getTableFieldsName("subscription_type");
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_subscription_type" >
   ...
   <tr>
   <td> 
		<a href="subscription_type?cmd=edit" class="nav3">Add a subscription type</a>
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			<tr bgcolor="#ABCAE0">
		 ...
		 ?>
			<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
             <td>
			  ...
			  <td><?=$arr[$i]['plan']?></td>
              <td><?=$arr[$i]['susbcription']?></td>
			  <td><?=$arr[$i]['customers']?></td>
			  <td><?=$arr[$i]['cards']?></td>
			  <td><?=$arr[$i]['status']?></td>
			  
			  <td nowrap >
                  <a href="subscription_type?cmd=edit&id=<?=$arr[$i]['id']?>" class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a> 
                  <a href="subscription_type?cmd=delete&id=<?=$arr[$i]['id']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
			 </td>
		   ...
		   <td colspan="10" align="center">
			  ...
			  <?php              
					  ...
					  $res  = $db->select($info);  
						...
						$self = 'subscription_type?cmd=list';
						$nav  = '';
