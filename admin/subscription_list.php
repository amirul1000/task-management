...
?>   
			  ...
			  <form name="search_frm" id="search_frm" method="post">
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					<TD  nowrap="nowrap">
					  <?php
						  $hash    =  getTableFieldsName("subscription");
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_subscription" >
   ...
   <tr>
   <td> 
		<a href="subscription?cmd=edit" class="nav3">Add a subscription</a>
		 <table class="table table-striped table-bordered table-hover" id="sample_1">
			<tr bgcolor="#ABCAE0">
              <td>Id</td>
			  <td>Users </td>
			 ...
			 <td><?=$arr[$i]['id']?></td>
			  ...
			  <td>
					<?php
                        ...
                        $info2['table']    = "plan";	
                        $info2['fields']   = array("*");	   	   
                        $info2['where']    =  "1 AND id='".$arr[$i]['plan_id']."' LIMIT 0,1";
                        $res2  =  $db->select($info2);
                        echo $res2[0]['plan_name'];	
                    ?>
               </td>
			  <td><?=date("Y-m-d H:i:s",$arr[$i]['current_period_start'])?></td>
			  <td><?=date("Y-m-d H:i:s",$arr[$i]['current_period_end'])?></td>
			  
			  <td nowrap >
                  <a href="subscription?cmd=edit&id=<?=$arr[$i]['id']?>" class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a> 
                  <a href="subscription?cmd=delete&id=<?=$arr[$i]['id']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
			 </td>
