...
?>   
			  ...
			  <form name="search_frm" id="search_frm" method="post">
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					<TD  nowrap="nowrap">
					  <?php
						  $hash    =  getTableFieldsName("task_performed");
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_task_performed" >
   ...
   <tr>
   <td> 
		<a href="task_performed?cmd=edit" class="nav3">Add a task_performed</a>
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			<tr bgcolor="#ABCAE0">
			  <td>Task </td>
			  <td>Users </td>
			...
			<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
			  <td>
					<?php
                        ...
                        $info2['table']    = "task";	
                        $info2['fields']   = array("*");	   	   
                        $info2['where']    =  "1 AND id='".$arr[$i]['task_id']."' LIMIT 0,1";
                        $res2  =  $db->select($info2);
                        echo $res2[0]['task_name'];	
                    ?>
               </td>
			  <td>
					<?php
			  ...
			  <td nowrap >
                  <a href="task_performed?cmd=edit&id=<?=$arr[$i]['id']?>" class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a> 
                  <a href="task_performed?cmd=delete&id=<?=$arr[$i]['id']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
			 </td>
