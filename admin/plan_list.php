...
<div class="portlet box green">
				...
				<table width="60%" border="0"  cellpadding="0" cellspacing="0" class="bodytext">
				  <TR>
					...
					<td nowrap="nowrap" align="right">
					  <input type="hidden" name="cmd" id="cmd" value="search_plan" >
   ...
   <tr>
   <td> 
		<a href="plan?cmd=edit" class="nav3">Add a plan</a>
		<table class="table table-striped table-bordered table-hover" id="sample_1">
			<tr bgcolor="#ABCAE0">
                <td>Plan Name</td>
                <td>Price</td>
		 ...
		 ?>
			<tr bgcolor="<?=$row?>" onmouseover=" this.style.background='#ECF5B6'; " onmouseout=" this.style.background='<?=$row?>'; ">
			  ...
			  <td><?=$arr[$i]['price']?></td>
			  <td><?=$arr[$i]['no_of_company_allow']?></td>
			  <td><?=$arr[$i]['no_of_tasks_allow']?></td>
			  <td><?=$arr[$i]['no_of_contractor_report']?></td>
			  <td><?=$arr[$i]['subscription_duration_days']?></td>
              <td><?=$arr[$i]['description']?></td>
			  <td><?=$arr[$i]['status']?></td>
			  <td nowrap >
                  <a href="plan?cmd=edit&id=<?=$arr[$i]['id']?>" class="btn default btn-xs purple"><i class="fa fa-edit"></i>Edit</a> 
                  <a href="plan?cmd=delete&id=<?=$arr[$i]['id']?>" class="btn btn-sm red filter-cancel"  onClick=" return confirm('Are you sure to delete this item ?');"><i class="fa fa-times"></i>Delete</a> 
			 </td>
